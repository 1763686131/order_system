...
# ---------------------------------------------------------
# 🎯 核心路径自适应：完美兼容 Windows 调试与 NAS Docker 挂载
# ---------------------------------------------------------
...
MATERIALS_FILE = os.path.join(DATA_DIR, 'material_db.json')


# ---------------------------------------------------------
# 🎯 究极防弹数据库自愈逻辑
# 彻底免疫空文件、乱码、权限不足导致的服务瘫痪
# ---------------------------------------------------------
def read_users():
...
def change_user_password(username):
    if request.headers.get('Role') != 'super_admin': return jsonify({"message": "越权"}), 403
    req_data = request.json or {}
    p = str(req_data.get('password')).strip()
    users = read_users()
    for x in users:
...
def open_browser():
    if not os.path.exists(FRONTEND_PATH): webbrowser.open("http://localhost:7899")
