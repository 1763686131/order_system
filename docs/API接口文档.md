...
该接口用于后台顶栏通讯录，只返回启用部门以及在职、试用期或休假员工的公开联系
信息，不返回身份证、家庭住址、紧急联系人、账号权限等人事或安全字段。员工属于多个
部门时会出现在对应的多个部门中；没有有效部门归属的员工由前端归入“未分配部门”。
...
`online` 表示员工存在未撤销、未过期的登录会话，并且最近 10 分钟内有服务端记录的
活动。服务端最多每 5 分钟更新一次会话活动时间；没有登录账号、会话已失效或超过
在线窗口的员工返回 `false`。
...
2. `canAccessAdmin` + `admin.route.*`：前者允许进入后台，后者按业务权限栏目控制具体页面访问，侧栏大类在存在可访问分支时显示。
...
接口要求有效的后台登录 Session。浏览器应通过 `EventSource` 建立长连接并携带 Cookie。
同一个后台页面中的留言列表、审核通知和聊天窗口应共享一条连接，避免每个组件分别占用连接。

| 事件名 | 触发时机 | `data` 摘要字段 |
| --- | --- | --- |
| `connected` | 首次连接或浏览器自动重连成功 | `messages`、`notifications` 两组摘要 |
...
- 服务端约每 `1` 秒检查一次当前用户的消息和通知摘要。
- 空闲时每 `15` 秒发送 SSE 注释保活，不触发业务事件。
- 响应提供 `retry: 3000`，异常断开后浏览器约 `3` 秒自动重连。
- 连接保持期间每 `60` 秒刷新当前会话的 `last_seen_at`，供通讯录在线状态使用。
- 前端仍应保留低频 HTTP 查询作为降级通道；当前实现使用 `60` 秒间隔。

反向代理部署要求：关闭该路径的响应缓冲，保持长连接并将读取超时设置为大于保活周期。
接口响应包含 `Cache-Control: no-cache, no-transform` 和 `X-Accel-Buffering: no`，但 Nginx、
...
后台物流列表的“复制”内容使用一份服务器共享配置，不再读写浏览器 `localStorage`。模板管理在角色组权限页面的“复制模板管理”分类配置。顶部入口位于通知铃铛左侧，只有同时拥有 `admin.logistics_copy.entry`（显示入口）与 `admin.logistics_copy.read`（查看模板）的后台账号才显示入口并可打开设置弹窗；超级管理员不受单项权限限制。创建、修改、删除分别需要 `admin.logistics_copy.create`、`admin.logistics_copy.edit`、`admin.logistics_copy.delete`。无对应权限的控件不可操作，服务端也会对实际变更再次校验。既有后台角色在首次迁移时会自动获得这五项权限，之后可在角色组管理中按需收回。首次没有服务端模板数据时，“历史模板”为空；新增、删除、绑定入口、绑定人和字段调整只修改当前弹窗草稿，点击“保存设置”后才写入数据库。

模板可以绑定复制入口和多个账号。当前复制入口包括 `logistics-info`（物流信息）和 `order-info`（订单信息复制）。同一个复制入口可以被不同账号分别绑定不同模板；同一个账号在同一个复制入口上只能绑定一个模板。复制时服务端按当前会话账号 ID 查找匹配模板；未找到绑定时回退到共享 `fields` 配置。绑定人列表仅由管理接口返回，包含账号头像和名称，前端只展示头像，鼠标悬浮显示姓名；普通复制接口不会返回其他账号或模板列表。

### 19.1 读取配置

- **URL**: `/api/settings/logistics-copy`
- **Method**: `GET`
- **权限**: 后台访问权限，且同时拥有 `admin.logistics_copy.entry` 与 `admin.logistics_copy.read`
...
首次配置未保存时返回 `fields: null`、`templates: null`、`updatedAt: null`，前端使用内置默认字段和默认历史模板；这些默认模板只存在于当前弹窗草稿，点击“保存设置”后才会写入服务器。已保存空字段列表时返回 `fields: []`，复制结果为空，不会恢复默认。已保存空模板列表时返回 `templates: []`，模板列表显示为空。打开设置面板会重新读取；接口读取失败时不使用旧缓存进行复制。

### 19.2 读取当前账号复制格式

- **URL**: `/api/settings/logistics-copy/resolve?target=logistics-info` 或 `?target=order-info`
- **Method**: `GET`
- **权限**: 后台访问权限；`logistics-info` 需要 `admin.route.sales.logistics`，`order-info` 需要 `admin.route.sales.orders`。无需复制模板管理权限。
...
服务端根据会话账号选择绑定模板或共享字段，忽略 URL 中的任何用户 ID；响应不会包含模板列表和绑定用户。未配置共享字段时 `fields` 为 `null`，前端采用内置默认字段；显式配置为空数组时保持为空。非法 `target` 返回 `400`，未登录返回 `401`，无对应路由权限返回 `403`。

### 19.3 保存配置

- **URL**: `/api/settings/logistics-copy`
- **Method**: `PUT`
- **权限**: 后台访问权限与 `admin.logistics_copy.entry`、`admin.logistics_copy.read`；还需拥有实际变更对应的 `create`、`edit`、`delete` 权限之一
- **Content-Type**: `application/json`
...
数据库连接初始化时自动创建表；旧表启动时自动补充 `templates_json` 字段；`id=1` 保证只有一份共享配置。`fields_json` 保存当前复制字段，首次只新增模板时保存 JSON `null`；`templates_json` 保存模板名称、说明、复制入口、绑定账号和模板字段数组，保存时通过 upsert 更新，无需手动迁移表结构。数据库初始不插入默认模板，未初始化时 `templates_json` 为 `NULL`，只有点击“保存设置”后才写入 `[]` 或实际模板数组。管理读取接口同时返回 `bindingUsers`，仅包含活动账号的 ID、展示名、用户名和头像地址。旧浏览器 `admin_logistics_copy_fields` 不会自动导入服务器，也不再被前端读取或写入。
...
页面访问权限放在对应的业务权限栏目中，不单独显示“后台路由”分类。销售订单页面访问项位于“销售订单操作”，
物流列表访问项位于“物流管理”；后续可在相同业务栏目中继续增加新增、编辑、删除等操作权限。

| 权限编码 | 权限栏目 | 控制范围 |
|---|---|---|
| `admin.route.dashboard` | 后台首页 | 后台首页 |
| `admin.route.products.list` | 商品管理 | 商品列表 |
| `admin.route.products.materials` | 商品管理 | 原材料列表 |
| `admin.route.sales.orders` | 销售订单操作 | 销售订单页面及订单表单 |
| `admin.route.sales.logistics` | 物流管理 | 物流操作列表 |
| `admin.route.sales.returns` | 退货管理 | 退货订单及表单 |
| `admin.route.sales.customers` | 客户管理 | 客户列表 |
| `admin.route.purchase.orders` | 采购订单 | 采购订单列表 |
| `admin.route.purchase.suppliers` | 供应商管理 | 供应商列表 |
| `admin.route.purchase.inbound` | 采购入库 | 采购入库列表 |
| `admin.route.inventory.products` | 成品库存 | 成品库存 |
| `admin.route.inventory.materials` | 原材料库存 | 原材料库存 |
| `admin.route.inventory.material_outbounds` | 原材料出库 | 原材料出库列表 |
| `admin.route.inventory.stock_in` | 库存流水 | 入库记录 |
| `admin.route.inventory.stock_out` | 库存流水 | 出库记录 |
| `admin.route.inventory.warehouse` | 仓库管理 | 仓库管理 |
| `admin.route.finance.receivables` | 应收管理 | 应收欠款和债务详情 |
| `admin.route.finance.payment_history` | 收款历史 | 收款历史 |
| `admin.route.finance.bank_accounts` | 银行账户 | 银行账户 |
| `admin.route.finance.logistics_truck` | 物流专车对账 | 物流/专车对账 |
| `admin.route.finance.express_courier` | 快运快递对账 | 快运/快递对账 |
| `admin.route.hr.company` | 公司资料 | 公司资料 |
| `admin.route.hr.reports` | 检测报告 | 检测报告 |
| `admin.route.system.stores` | 门店管理 | 门店管理 |
...
- 多个后台消息组件共享单个 `EventSource`，完整数据继续通过原有鉴权 HTTP 接口读取
- 增加 15 秒保活、3 秒自动重连提示、Session 失效事件和 60 秒前端降级查询
