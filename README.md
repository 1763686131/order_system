...
当前聊天功能尚未实现。目标是：在通讯录中双击在线员工，打开一个简单的好友聊天窗口，
支持文字消息和局域网文件传输，不追求微信级别的群聊、朋友圈、语音或复杂会话能力。

#### 实现难度

整体难度为中等，文字聊天本身不难，真正需要认真处理的是实时连接、在线状态、
消息可靠性、文件权限和异常断线。按照当前项目已有的 Flask、SQLite、Session
和上传文件结构，建议先做一个一对一聊天 MVP，再逐步增加历史记录、未读数量和文件预览。

#### 推荐架构

```text
浏览器 A ─┐
          ├─ 局域网 Flask 服务 ─ SQLite + uploads/chat-files/
浏览器 B ─┘
```

- 浏览器只连接同一个局域网业务服务器，消息和文件都由服务器转发或保存。
- 实时文字消息建议增加 WebSocket 通道；现有 HTTP API 继续负责历史消息、
  文件上传、文件下载和权限校验。
- 不建议第一版采用浏览器之间的 WebRTC 点对点传输。它需要额外处理连接协商、
  防火墙、断线重连和文件权限，复杂度明显高于当前需求。
- “双方在线”建议以服务端在线会话和 WebSocket 心跳为准。现有通讯录的
  `10` 分钟最近活动状态可以继续展示，但不能单独作为聊天连接是否可用的依据。
- 系统已经运行在内网时，通常不需要额外判断两个浏览器的 IP 是否完全相同网段；
  只要双方都通过同一个内网服务登录并保持在线即可。若必须严格限制网段，应在服务端
  配置允许的内网 CIDR，并对会话来源地址做校验，不要只依赖前端判断。
...
| 表 | 作用 |
| --- | --- |
| `chat_conversations` | 一对一会话，保存两个员工 ID、创建时间和最后消息时间 |
| `chat_messages` | 消息正文、发送人、接收人、发送时间、消息状态和撤回标记 |
| `chat_attachments` | 文件名、保存路径、大小、MIME 类型、上传人和消息关联 |
| `chat_message_reads` | 可选，用于记录已读时间和未读数量 |

文件实际保存到 `uploads/chat-files/YYYY-MM/`，数据库只保存随机文件名、
访问路径和文件元数据。下载接口必须由后端验证当前 Session 是否属于该会话，
不能把文件目录直接暴露为任意可访问的静态目录。
...
| 组件 | 职责 |
| --- | --- |
| `DirectoryPanel.vue` | 通讯录搜索、部门折叠和员工列表；双击员工后发出打开聊天事件 |
| `ChatWindow.vue` | 单个好友的聊天窗口、标题栏、在线状态和关闭操作 |
| `ChatMessageList.vue` | 消息列表、时间分组、发送状态和滚动到底部 |
| `ChatComposer.vue` | 文字输入、发送、选择文件和上传进度 |
| `src/api/chat.js` | 历史消息、文件上传、文件下载和会话接口 |
| `src/stores/chat.js` | 当前会话、未读数量、连接状态和重连状态 |

#### 推荐开发顺序

1. 增加一对一会话和文字消息表，完成历史消息接口。
2. 增加 WebSocket 连接、心跳、重连和消息回执。
3. 在 `DirectoryPanel.vue` 的员工项上增加双击事件，只允许选择有聊天资格的员工。
4. 新增聊天窗口，实现文字发送、接收、失败重试和未读计数。
5. 增加局域网文件上传、进度显示、文件大小和类型校验、下载权限校验。
6. 补充断线、刷新页面、对方退出、重复发送、文件上传失败等异常处理。

#### 第一版建议限制

- 只支持一对一文字聊天，不做群聊、语音、视频和消息撤回。
- 文件先限制单个文件大小，例如 `50 MB`，具体值放到系统配置中。
- 允许常见办公文件和图片，服务端按实际文件内容和扩展名双重校验。
- 采用随机存储文件名，原始文件名只作为展示信息保存。
- 双方不在线时不建立发送通道；可以选择允许查看历史，但发送按钮置灰。
- 聊天窗口关闭不代表退出会话，消息仍由服务端保存，重新打开后继续加载。

这个方案的核心优点是复用现有登录、权限、SQLite 和 `uploads/` 体系。第一版完成后，
再增加未读消息、文件预览、最近会话列表和消息搜索，不需要推翻基础架构。
